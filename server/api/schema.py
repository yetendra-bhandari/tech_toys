...
def resolve_getSeller(_, info):
    if('Authorization' in info.context.headers):
        return queries.getSeller(info.context.headers['Authorization'])
    return None
...
def resolve_getSellerProducts(_, info):
    if('Authorization' in info.context.headers):
        return queries.getSellerProducts(info.context.headers['Authorization'])
    return None
...
def resolve_getSellerPendingOrders(_, info):
    if('Authorization' in info.context.headers):
        return queries.getSellerPendingOrders(info.context.headers['Authorization'])
    return None
...
def resolve_getCustomer(_, info):
    if('Authorization' in info.context.headers):
        return queries.getCustomer(info.context.headers['Authorization'])
    return None
...
def resolve_getCustomerAddresses(_, info):
    if('Authorization' in info.context.headers):
        return queries.getCustomerAddresses(info.context.headers['Authorization'])
    return None
...
def resolve_getCustomerCartItems(_, info):
    if('Authorization' in info.context.headers):
        return queries.getCustomerCartItems(info.context.headers['Authorization'])
    return None
...
def resolve_getCustomerOrders(_, info):
    if('Authorization' in info.context.headers):
        return queries.getCustomerOrders(info.context.headers['Authorization'])
    return None
...
def resolve_refreshToken(_, info):
    if('Authorization' in info.context.headers):
        return queries.refreshToken(info.context.headers['Authorization'])
    return None
...
def resolve_editAccountImage(_, info, image=None):
    if('Authorization' in info.context.headers):
        return queries.editAccountImage(info.context.headers['Authorization'], image)
    return None
...
def resolve_editSeller(_, info, sellerInput, password):
    if('Authorization' in info.context.headers):
        return queries.editSeller(info.context.headers['Authorization'], sellerInput, password)
    return None
...
def resolve_addSellerProduct(_, info, productInput):
    if('Authorization' in info.context.headers):
        return queries.addSellerProduct(info.context.headers['Authorization'], productInput)
    return None
...
def resolve_editCustomer(_, info, customerInput, password):
    if('Authorization' in info.context.headers):
        return queries.editCustomer(info.context.headers['Authorization'], customerInput, password)
    return None
...
def resolve_addCustomerAddress(_, info, addressInput):
    if('Authorization' in info.context.headers):
        return queries.addCustomerAddress(info.context.headers['Authorization'], addressInput)
    return None
...
def resolve_addCustomerCartItem(_, info, productID, quantity):
    if('Authorization' in info.context.headers):
        return queries.addCustomerCartItem(info.context.headers['Authorization'],  productID, quantity)
    return None
...
def resolve_deleteCustomerCartItem(_, info, cartItemID):
    if('Authorization' in info.context.headers):
        return queries.deleteCustomerCartItem(info.context.headers['Authorization'],  cartItemID)
    return None
...
def resolve_addCustomerOrders(_, info, cartItemIDs, addressID):
    if('Authorization' in info.context.headers):
        return queries.addCustomerOrders(info.context.headers['Authorization'],  cartItemIDs, addressID)
    return None
...
def resolve_editCustomerOrderStatus(_, info, orderID, status):
    if('Authorization' in info.context.headers):
        return queries.editCustomerOrderStatus(info.context.headers['Authorization'],  orderID, status)
    return None
...
def resolve_editProductDiscount(_, info, productID, discount):
    if('Authorization' in info.context.headers):
        return queries.editProductDiscount(info.context.headers['Authorization'],  productID, discount)
    return None
...
def resolve_editProductStock(_, info, productID, inStock):
    if('Authorization' in info.context.headers):
        return queries.editProductStock(info.context.headers['Authorization'],  productID, inStock)
    return None
...
def resolve_addProductReview(_, info, productID, reviewInput):
    if('Authorization' in info.context.headers):
        return queries.addProductReview(info.context.headers['Authorization'],  productID, reviewInput)
    return None
...
def resolve_deleteProductReview(_, info, reviewID):
    if('Authorization' in info.context.headers):
        return queries.deleteProductReview(info.context.headers['Authorization'],  reviewID)
    return None
